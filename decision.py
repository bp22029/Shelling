...
from schelling_core import TEAM_A, PREF_LOW, PREF_MID, PREF_HIGH


class DecisionMaker(ABC):
    @abstractmethod
    def decide(self, team: int, same: int, diff: int, empty: int,
               preference: str) -> str:
        """'stay' または 'move' を返す。"""
        ...
...
class RuleFixedDecision(DecisionMaker):
    ...
    def decide(self, team, same, diff, empty, preference):
        total = same + diff
        if total == 0:
            return "stay"
        ratio = same / total
...
class LLMDecision(DecisionMaker):
    ...
    def build_prompt(self, team, same, diff, empty, preference):
        type_name = "A" if team == TEAM_A else "B"
        base_situation = (
            f"あなたの周囲には以下の住民がいます。\n"
            f"- Aタイプ: {same if team == TEAM_A else diff}人\n"
            f"- Bタイプ: {diff if team == TEAM_A else same}人\n"
            f"- 空き地: {empty}つ\n\n"
            f"あなたは現在の場所に住み続けますか。\n"
            f"それとも移動しますか。\n\n"
    ...
    def decide(self, team, same, diff, empty, preference):
        prompt = self.build_prompt(team, same, diff, empty, preference)
        self.call_count += 1
