...
### 更新方式は「逐次（非同期）」で固定（重要な設計方針）

`run_simulation` は、1ステップ内でエージェントを1体ずつ処理し、各エージェントは
**直前までの移動が反映された最新の盤面**を見て判断する（逐次／非同期更新）。これは
シェリングの逐次的な再配置に忠実で、「個体の移動が後続の判断に波及する」性質を保つ。

- ステップ内を**同期更新（スナップショット一斉判断）に変えない**こと。波及が消え、別モデルになる。
- 1ステップ内の判断は互いに依存するため、**忠実性を保つ限りステップ内は並列化できない**。
  高速化が必要なときは、独立したラン／試行／条件（seed・選好分布・モデル・Exp1〜3 など）を
  **同時実行**して並列度を稼ぐ（各ランの中身は逐次のままで結果は不変）。
